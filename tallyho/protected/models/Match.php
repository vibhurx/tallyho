...
class Match extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('level, sequence, event_id, category', 'required'),
			array('level, sequence, player11, player12, player21, player22, winner, event_id, category', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, level, sequence, player11, player12, player21, player22, winner, event_id, category', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'enrolment11' => array(self::BELONGS_TO, 'Enrolment', 'player11'),
 			'enrolment21' => array(self::BELONGS_TO, 'Enrolment', 'player21'),
 			'enrolment12' => array(self::BELONGS_TO, 'Enrolment', 'player12'),
 			'enrolment22' => array(self::BELONGS_TO, 'Enrolment', 'player22'),
			'event' => array(self::BELONGS_TO, 'Event', 'event_id'),
			//'umpire' => array(self::BELONGS_TO, 'Contact', 'scorer'),
					
// 			'category' => array(self::BELONGS_TO, 'Category', 'category'),
// 			'enrolments' => array(self::HAS_MANY, 'Enrolment', 'match_id'),
		);
	}
	...
	public function promoteWinner(){
		...
		$category = Category::model()->findByAttributes(array('event_id'=>$this->event_id, 'category'=> $this->category));
		...
		if($this->level > 0){
			$next_match = Match::model()->findByAttributes(array(
					'event_id'=>$this->event_id, 'category'=> $this->category, 'sequence'=>$new_seq));
			...
			$next_match = Match::model()->findByAttributes(array(
					'event_id'=>$this->event_id, 'category'=> $this->category, 'sequence'=>$new_seq, 'level'=>'0'));
		}
	...
	*	--------------------------------------------------------------------------------------------*/
	public function checkAuthorization($pUsername){
		try {
			$tour_organizer = Event::model()->findByPk($this->event_id)->org_id;
			...
			$contact_organizer = $contact->org_id;
			if($tour_organizer != $contact_organizer){
