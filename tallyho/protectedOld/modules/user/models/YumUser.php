...
class YumUser extends YumActiveRecord
{
	...
	public function relations() {
		...
		if($relations === false) {
			...
			//Following 2 relationships are added by MUBI
// 			if (Yum::hasModule('contact')) {
// 				Yii::import('application.modules.contact.models.*');
// 				$relations['contact'] = array(self::HAS_ONE, 'Contact', 'user_id'); //,
// 			}
			
			if (Yum::hasModule('player')) {
				Yii::import('application.modules.player.models.*');
				$relations['player'] = array(self::HAS_ONE, 'Player', 'user_id'); //,
			}
			if (Yum::hasModule('developer')) {
				Yii::import('application.modules.developer.models.*');
				$relations['developer'] = array(self::HAS_ONE, 'Developer', 'user_id'); //,
			}
	...
	public function register($username = null,
  		...
  		if (!($profile instanceof YumProfile)){
  			Yum::setFlash('The profile object does not seem to be an instance of YumProfile class. 
  					It is actually an instance of ' . get_class($profile));
  			return false;
 		}
...
// 			Yum::setFlash('username and password are not null');
// 			return false;
...
// 		Yum::setFlash('Line 578... no problem');
// 		return false;
...
// 		Yum::setFlash('Line 594... no problem');
// 		return false;
		...
		Yum::setFlash('Line 522... validation did not go fine :-(');
	...
	public static function activate($email, $key) {
		...
		//if($user = YumUser::model()->find("email = :email", array(':email' => $email))) {
	...
	public function generateActivationKey($activate = false) {
		...
		$this->activationKey = str_replace('/', '_', $this->activationKey);		//<<< MUBI CR_2015_20_001
